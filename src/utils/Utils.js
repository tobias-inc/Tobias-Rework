...
module.exports = class Utils {
  ...
  static reactionCollectorResponse (
    message,
    filter,
    { clear, ...options } = {
      time: 30000,
    ...
    if (message instanceof Message) {
      return new Promise(resolve => {
        ...
        const clearReactions = () => message.reactions.removeAll().catch(() => null)
        collector
          .on('collect', () => {
  ...
  static guildSend (message, channelId, { client }) {
    ...
    const guild = client.guilds.cache.get(Constants.BOT_GUILD)
