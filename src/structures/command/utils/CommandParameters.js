...
const funcOrString = (funct, stringfunc, ...args) => {
  typeof funct === 'function' ? funct(...args) :  typeof stringfunc === 'function' ? stringfunc(funct) : funct
  }
const normalizeParam = p => {
  const type = ParameterTypes[p.type] || p.type
  if (!type || !(type.prototype instanceof Parameter)) {
...
module.exports = class CommandParameters {
  static parseOptions (params = []) {
    ...
    return {
      ...
      parameters: (hasFlags ? params.slice(0, length - 1) : params).map(
        normalizeParam
      )
    }
  }
    ...
    if (opts.flags) {
      ...
      if (flagIndex > -1) {
        ...
        for (let i = 0; i < flagsParsed.length; i++) {
          ...
          const parsedFlag = await this.parseParameter(
            context,
            flag,
            flagValue,
            missingErr
  ...
  static async handleArguments (context, opts, args) {
    ...
    for (let i = 0; i < opts.parameters.length; i++) {
      ...
      if (
        ...
        !param.required &&
        parseState.argIndex === args.length - 1 &&
      ...
      if (param.full) {
        arg = args.slice(parseState.argIndex).join(param.fullJoin || ' ')
      }
      ...
      const parsedArg = await this.parseParameter(
        ...
        param,
        arg,
        funcOrString(param.missingError, context.t, context)
      )
