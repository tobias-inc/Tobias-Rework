...
module.exports = class CommandRequirements {
  ...
  static async handle ({ t, author, channel, client, command, guild, member, message 
    },
    options
  ) {
    ...
    if (command.cooldownFeedback && !await PermissionUtils.isDeveloper(client, author)) {
      if (command.cooldown.has(author.id)) {
        throw new CommandError(t(opts.errors.cooldown))
      }
    }

    if (opts.devOnly && !await PermissionUtils.isDeveloper(client, message.member)) {
    ...
    const guildPlayer = client.playerManager // && client.playerManager.cache.get(guild.id)

    if (opts.guildPlaying && (!guildPlayer || !guildPlayer.playing)) {
      throw new CommandError(t(opts.errors.guildPlaying))
    }

    if (opts.sameVoiceChannelOnly && guild.me.voiceChannelID && (!member.voice.channel || guild.me.voice.channel.id !== member.voice.channel.id)) {
      throw new CommandError(t(opts.errors.sameVoiceChannelOnly))
    }
    if (opts.voiceChannelOnly && !member.voice.channel) {
